    ...
    for L in range(0,len(GPGSV)):    
        ...
        for L2 in satPlace:
            if GPGSV[L][L2] == satelite:
                intensidad = GPGSV[L][L2+3]
                print("intensidad ", intensidad)
                angAzim = GPGSV[L][L2+2]
                print("angAzim ", angAzim)
                angElev = GPGSV[L][L2+1]
                print("angElev ", angElev)

    return angElev, angAzim, intensidad


if __name__ == "__main__":
    linea = '$GPGSV,1,1,02,12,00,000,34,18,00,000,00*76'
    satelite = 12
    parspy(linea, satelite)
